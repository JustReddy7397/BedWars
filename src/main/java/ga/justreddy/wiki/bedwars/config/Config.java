package ga.justreddy.wiki.bedwars.config;
...
public class Config {
     ...
     */
    public Config(final BedWars plugin, final String name)
    ...
    public boolean isOutdated(final int currentVersion) {
        if (config.getInt(VERSION_KEY, -1) < currentVersion) {
            plugin.getLogger().severe("Outdated config file version: " + currentVersion);
